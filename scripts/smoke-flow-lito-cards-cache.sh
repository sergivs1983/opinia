...
LITO_CARDS_CACHE_COOKIE="${LITO_CARDS_CACHE_COOKIE:-}"
LITO_CARDS_CACHE_BIZ_ID="${LITO_CARDS_CACHE_BIZ_ID:-}"
...
perform_request() {
  local resp
  resp="$(curl -sS -w $'\n%{http_code}\n%{time_total}' --max-time 30 "$@" 2>/dev/null || true)"
  REQ_TIME="$(printf '%s\n' "$resp" | tail -n 1)"
  REQ_CODE="$(printf '%s\n' "$resp" | tail -n 2 | head -n 1)"
  REQ_BODY="$(printf '%s\n' "$resp" | sed '$d' | sed '$d')"
}
...
report_fail() {
  echo "  [${FAIL}] $1"
  echo "         HTTP=${REQ_CODE} TIME=${REQ_TIME}s"
...
is_fast_enough() {
  local seconds="$1"
  awk -v t="$seconds" 'BEGIN { if (t+0 < 1.5) exit 0; exit 1 }'
}

echo "Flow LITO Cards Cache smoke — ${BASE}"
echo "────────────────────────────────────────────────────────────────────────"
...
echo ""
echo "2) Functional shape + speed (opcional)"
if [ -n "${LITO_CARDS_CACHE_COOKIE}" ] && [ -n "${LITO_CARDS_CACHE_BIZ_ID}" ]; then
  COOKIE_HEADER="$(normalize_cookie_header "${LITO_CARDS_CACHE_COOKIE}" || true)"
  if [ -z "${COOKIE_HEADER}" ]; then
    REQ_CODE="cookie"
    REQ_BODY="LITO_CARDS_CACHE_COOKIE invàlida"
    REQ_TIME="n/a"
    report_fail "cookie format"
  else
    perform_request "${BASE}/api/lito/action-cards?biz_id=${LITO_CARDS_CACHE_BIZ_ID}" \
      -H "${COOKIE_HEADER}"
    ...
    if [ "${REQ_CODE}" = "200" ]; then
      ...
      if is_fast_enough "${REQ_TIME}"; then
        report_ok "response ràpida (<1.5s, got ${REQ_TIME}s)"
      else
        report_fail "response ràpida (expected <1.5s)"
      fi
    else
  ...
  report_ok "SKIP functional (defineix LITO_CARDS_CACHE_COOKIE i LITO_CARDS_CACHE_BIZ_ID)"
fi
