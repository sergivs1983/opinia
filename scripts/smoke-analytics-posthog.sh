...
echo "Analytics PostHog smoke — ${BASE}"
echo "────────────────────────────────────────────────────────────────────────"
...
perform_request "${BASE}/api/analytics/capture" \
  ...
  -d '{"biz_id":"00000000-0000-0000-0000-000000000000","event":"open_app"}'
...
echo ""
echo "2) Functional (opcional)"
if [ -z "${POSTHOG_KEY}" ]; then
  report_ok "functional SKIP (POSTHOG_PROJECT_API_KEY no definit a shell)"
elif [ -z "${ANALYTICS_SESSION_COOKIE}" ] || [ -z "${ANALYTICS_BIZ_ID}" ]; then
  report_ok "functional SKIP (defineix ANALYTICS_SESSION_COOKIE + ANALYTICS_BIZ_ID)"
else
  ...
  if [ -z "${COOKIE_HEADER}" ]; then
    ...
    perform_request "${BASE}/api/analytics/capture" \
      ...
      -d "{\"biz_id\":\"${ANALYTICS_BIZ_ID}\",\"event\":\"open_app\",\"mode\":\"basic\",\"properties\":{\"source\":\"smoke\"}}"
    if [ "${REQ_CODE}" = "200" ]; then
      report_ok "analytics capture funcional (HTTP 200)"
    else
      report_fail "analytics capture funcional (expected 200)"
    fi
  fi
fi
