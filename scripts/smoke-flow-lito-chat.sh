...
LITO_CHAT_COOKIE="${LITO_CHAT_COOKIE:-${LITO_SESSION_COOKIE:-}}"
LITO_CHAT_BIZ_ID="${LITO_CHAT_BIZ_ID:-${LITO_BIZ_ID:-}}"
...
validate_stream_shape() {
  local body="$1"
  if printf '%s' "$body" | grep -q 'event: meta' && printf '%s' "$body" | grep -q 'event: done'; then
    return 0
  fi
  return 1
}

echo "Flow LITO Chat stream smoke — ${BASE}"
echo "────────────────────────────────────────────────────────────────────────"
...
perform_request -X POST "${BASE}/api/lito/chat" \
  ...
  -d '{"biz_id":"00000000-0000-0000-0000-000000000000","message":"hola"}'
...
if [ -n "${LITO_CHAT_COOKIE}" ] && [ -n "${LITO_CHAT_BIZ_ID}" ] && has_provider_key; then
  COOKIE_HEADER="$(normalize_cookie_header "${LITO_CHAT_COOKIE}" || true)"
  if [ -z "${COOKIE_HEADER}" ]; then
    REQ_CODE="cookie"
    REQ_BODY="LITO_CHAT_COOKIE invàlida"
    ...
    perform_request -X POST "${BASE}/api/lito/chat" \
      ...
      -d "{\"biz_id\":\"${LITO_CHAT_BIZ_ID}\",\"message\":\"Prepara'm una prioritat per avui\"}"

    if [ "${REQ_CODE}" = "200" ]; then
      if validate_stream_shape "${REQ_BODY}"; then
        report_ok "POST /api/lito/chat amb sessió (stream SSE OK)"
      else
        report_fail "POST /api/lito/chat stream shape (expected meta+done events)"
      fi
    else
      report_fail "POST /api/lito/chat amb sessió (expected 200)"
    fi
  fi
else
