...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const log = createLogger({ request_id: requestId, route: 'POST /api/cron/gbp-reviews-sync' });
  ...
  if (integrationError) {
    log.error('cron_gbp_reviews_integrations_query_failed', {
  ...
  for (const bizId of bizIds) {
    const body = JSON.stringify({ biz_id: bizId });
