...
export async function POST(
  ...
  const log = createLogger({ request_id: requestId, route: 'POST /api/lito/action-drafts/[id]/approve' });
  ...
  try {
    ...
    const ctx = await loadDraftContext({
      ...
      route: 'POST /api/lito/action-drafts/[id]/approve',
    });
    ...
    if (error || !data) {
      log.error('lito_action_draft_approve_failed', {
    ...
    log.error('lito_action_draft_approve_unhandled', {
