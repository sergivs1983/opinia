...
export default function LITOHealthTab() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(payload.message || 'No sha pogut carregar la telemetria.');
      }
      setSummary(payload);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : 'No sha pogut carregar la telemetria.');
    } finally {
      setLoading(false);
    }
  }, [canManage, org?.id]);
  ...
  const handleCopyDebug = async () => {
    ...
    try {
      ...
      toast('No sha pogut copiar el debug', 'error');
    }
  };
